...
module.exports = function(RED) {
    function Node(config) {
		...
		if(type === 0)
		{
			value = widgetlords.pi_spi_din_4freq_read_fixed(parseInt(config.chipenable), parseInt(config.address), parseInt(config.channel));
		}
		else if(type === 1)
		{
			value = widgetlords.pi_spi_din_4freq_read_variable(parseInt(config.chipenable), parseInt(config.address), parseInt(config.channel));
		}
		else if(type === 2)
		{
			value = widgetlords.pi_spi_din_4freq_read_pulse(parseInt(config.chipenable), parseInt(config.address), parseInt(config.channel));
		}
		else if(type === 3)
		{
			value = widgetlords.pi_spi_din_4freq_read_di(parseInt(config.chipenable), parseInt(config.address));
